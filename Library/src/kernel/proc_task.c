...
memdiff_view *proc_to_taskv_check(ks_proc_t proc, bool check, uint64_t proc_addr) {
  ...
  if ((proc_ro_v->pr_proc != (ks_proc_t)proc_addr) && check) {
...
memdiff_view *task_to_procv_check(ks_task_t task, bool check, uint64_t task_addr) {
  ...
  if (ptr_is_kernel((uint64_t)task) && check) {
    printf("task_to_proc: task pointer is kernel (expected memoryview): 0x%llx\n", (unsigned long long)task);
    return 0;
  }

  if (!ptr_is_kernel((uint64_t)task->bsd_info_ro) && check) {
  ...
  if ((task_bsd_info_v->pr_task != (ks_task_t)task_addr) && check) {
...
ks_task_t proc_to_task_check(ks_proc_t proc, bool check, uint64_t proc_addr) {
  memdiff_view *taskview = proc_to_taskv_check(proc, check, proc_addr);
  if (!taskview) {
      printf("proc_to_task: failed to create memdiff view for task\n");
      return 0;
  }
  ...
  if ((task_v->bsd_info_ro != proc->p_proc_ro) && check) {
...
ks_proc_t task_to_proc_check(ks_task_t task, bool check, uint64_t task_addr) {
  // create the memdiff for proc
  memdiff_view *procview = task_to_procv_check(task, check, task_addr);
  if (!procview) {
    printf("task_to_proc: failed to create memdiff view for proc\n");
    return 0;
  }
  ...
  if ((proc_v->p_proc_ro != task->bsd_info_ro) && check) {
