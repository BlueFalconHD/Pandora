...
static inline kern_return_t pandora_read(io_connect_t client, uint64_t from,
...
static inline kern_return_t pandora_get_kbase(io_connect_t client,
                                              uint64_t *out) {
  uint32_t outCnt = 1;
  return IOConnectCallScalarMethod(client, 1, NULL, 0, out, &outCnt);
}
...
static inline kern_return_t pandora_get_metadata(io_connect_t client,
  ...
  return IOConnectCallStructMethod(client, 2, NULL, 0, metadata, &outputSize);
}
